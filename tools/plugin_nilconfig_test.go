...
func TestPluginTools_NilConfigDoesNotPanicOnConstruction(t *testing.T) {
	...
	if NewPluginEnable(toolapi.NewRegistry(), nil, NewService(t.TempDir())) == nil {
		t.Fatal("want a tool")
	}
...
func TestPluginTools_NilConfigFailsWhenCalled(t *testing.T) {
	cases := map[string]toolapi.TypedExecutor{
		"plugin_enable": NewPluginEnable(toolapi.NewRegistry(), nil, NewService(t.TempDir())),
...
func TestPluginTools_NilConfigIsAGapNotACrash(t *testing.T) {
	...
	if msg.Type != "plugin" || msg.Status != toolapi.StatusError {
