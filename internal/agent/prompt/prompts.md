...
- "investigate": the turn needs the full agent — anything you cannot answer from your own knowledge or the conversation alone. That means fetching live or external data (current prices, today's news, the contents of a URL), using a tool or taking a real action (search, fetch, run code, read a file), working through a document or dataset the user supplied, or a task that takes several steps in sequence. A SINGLE request still counts if it needs a tool or live data — "what's the weather in Tokyo right now" is investigate, not chat, because chat cannot fetch it. Rule of thumb: if a correct answer requires acting on the world or fresh data beyond what you already know, it is investigate; otherwise it is chat. Merely naming a person, number, id, website, or topic is NOT by itself investigate.
