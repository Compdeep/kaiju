			...
			if comp.Err != nil {
				...
				if parseErr != nil {
					...
					case "replan":
						...
						frame := fmt.Sprintf(
							"\n\n## Re-plan\nThe plan so far has already run — the worklog below (## System State) shows completed work. Do NOT repeat completed steps.\n\nReflector says the next move is:\n%s\n\nPlan ONLY the next steps needed to close this gap and answer the original request above. Prior steps have ALREADY FINISHED and their results appear above as DATA — reference those values LITERALLY (paste the actual URL / text) and set `depends_on: []`. Do NOT use `${step.N.field}` or `depends_on:[N]` to reach a prior result: those indices address ONLY the new steps in THIS plan, never the completed ones (a stale index silently points a step at itself and it is skipped). Use `${step.N.field}` solely to wire one NEW step to another NEW step in this same plan.\n\nIf the next move is to FIX a FAILURE, plan a single `debug` step (a leaf, no dependents) with the failure — exact error text, file paths, module names — in its `problem` param; the debugger diagnoses the root cause and applies the fix, and the following re-plan handles any follow-on work. If the request is already fully answered by the worklog, return an empty plan.",
							next)
