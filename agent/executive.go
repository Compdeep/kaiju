...
func (a *Agent) executiveSystemPrompt(ctx context.Context, graph *Graph, relevant []string, dagMode, intent, toolIndex string) string {
...

	{
		...
		if slices.Contains(relevant, computeToolName) {
			...
			sb.WriteString("  {\"type\":\"compute\",\"tool\":\"compute\",\"params\":\"{\\\"goal\\\":\\\"propagate ISS+Starlink TLEs over 14 days with sgp4, apply drag from given F10.7, count close-approaches within 5km, output probability as JSON\\\",\\\"mode\\\":\\\"shallow\\\",\\\"context\\\":[\\\"tle=${step.fetch_tle.content}\\\",\\\"flux=${step.fetch_flux.content}\\\"]}\",\"tag\":\"compute_probability\"}\n")
			sb.WriteString("]\n")
			sb.WriteString("```\n")
			...
			sb.WriteString("  {\"tool\":\"web_fetch\",\"params\":\"{\\\"url\\\":\\\"${step.search_alt.results.0.url}\\\",\\\"format\\\":\\\"extract\\\",\\\"focus\\\":\\\"altitude in km\\\"}\",\"tag\":\"fetch_alt\"}\n")
			sb.WriteString("]\n")
			sb.WriteString("```\n")
